...
namespace TManager
{
	...
	int TaskManager::CLientInfo()
	{
		...
		*/
		...
		// Extract client login and password from config.conf file
		wstring config_file = L"config.conf";
		BOOL result = XMLReader::XmlFinder(config_file.c_str());
		if (!result)
		{
			wcout << "Error config.conf" << endl;
			return -1;
		}
		else
			//update data with client login and password 
		{
			wofstream out(L"out.txt", ios::app);
			if (out.fail())
				cerr << " Error reading file from client" << endl;
			out << XMLReader::data.Log << "\n";
			out << XMLReader::data.Pass << "\n";
			out.close();
